...
func GetCategory() {
	...
	c.OnHTML("ul.nav-exploded.explodedmenu", func(h *colly.HTMLElement) {
		...
		h.ForEach("li.itemMenu.subparent", func(_ int, e *colly.HTMLElement) {
			...
			if Find(listCate, Cate.URL) == -1 && Cate.URL != "https://www.acfc.com.vn" && !strings.Contains(Cate.URL, "?cat") && !strings.Contains(Cate.URL, "#") && Filter(Cate.URL) {
				...
				if len(tmp) == 4 {
					Cate.SubLevel = 0
					Cate.Name = Format(tmp[3])
					Cate.ParentID = uuid.Nil
				} else if len(tmp) == 5 {
					Cate.SubLevel = 1
					Cate.Name = Format(tmp[4])
					Cate.ParentID = GetParentID(listCate, tmp[3])
				} else if len(tmp) == 6 {
					Cate.SubLevel = 2
					Cate.Name = Format(tmp[5])
					Cate.ParentID = GetParentID(listCate, tmp[4])
				}
				Cate.ID = uuid.New()
...
func GetProductDetails() {
	...
	listProducDetail := []ProductDetail{}
	listProduct := []ProducInfo{}
	tmp := ""
